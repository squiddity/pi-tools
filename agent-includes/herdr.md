...
- Use `herdr_agent_send` to message a Herdr agent. Do not call raw `herdr agent send` through `bash`: it writes literal text without Enter, leaving the message in the child editor buffer.
- `herdr_jobs_list` prefixes ordinary entries with `job` and managed Pi entries with `agent`; use `herdr_job_read` for `job` entries and `herdr_agent_read` for `agent` entries. The latter accepts the listed managed-agent id, name, terminal id, or pane id.
