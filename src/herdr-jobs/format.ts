...
import type { PaneDisposition, RunningJob, TrackingDisposition } from "./types.ts";

export interface DeliveryDisposition {
  pane: PaneDisposition;
  tracking: TrackingDisposition;
}

function formatPaneLine(job: RunningJob, disposition: DeliveryDisposition): string {
  const pane = disposition.pane === "closed"
    ? "closed automatically"
    : disposition.pane === "missing"
      ? "missing"
      : disposition.pane === "retained"
        ? "retained for inspection"
        : disposition.pane;
  const next = disposition.tracking === "retained" && disposition.pane === "missing"
    ? "\nNext action: use herdr_job_close to forget the retained record."
    : "";
  return `Pane: ${job.metadata.paneId} (${pane})${next}`;
}
...
export async function formatResultMessage(job: RunningJob, exitCode: number, disposition: DeliveryDisposition): Promise<string> {
  ...
  return `${heading}\nExit code: ${exitCode}\n${formatPaneLine(job, disposition)}\nLog: ${job.paths.logFile}\n\nLast output:\n${tail.content}${tail.notice ? `\n\n${tail.notice}` : ""}`;
}

export async function formatFailureMessage(job: RunningJob, error: string, disposition: DeliveryDisposition): Promise<string> {
  const tail = await readLogTail(job.paths.logFile, 80);
